                ...
                {activeTab === 'overview' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="glass-card p-6 space-y-4">
                            <h3 className="font-semibold" style={{ color: 'var(--text-primary)' }}>Full Medical Info</h3>
                            {[
                                ['Phone', patient.phone],
                                ...
                                ['Admission Date', patient.admissionDate ? new Date(patient.admissionDate).toLocaleDateString() : null],
                                ['Discharge Date', patient.dischargeDate ? new Date(patient.dischargeDate).toLocaleDateString() : null],
                                ['Treatment Summary', patient.treatmentSummary],
                            ].map(([l, v]) => (
                                <div key={l} className="flex justify-between text-sm gap-2">
                                    <span className="flex-shrink-0" style={{ color: 'var(--text-muted)' }}>{l}</span>
                                    <span className="font-medium text-right break-words max-w-[70%]" style={{ color: 'var(--text-secondary)' }}>{v || 'N/A'}</span>
                                </div>
                            ))}
                        </div>
                        <div className="space-y-6">
                            <div className="glass-card p-6 space-y-4">
                                <h3 className="font-semibold" style={{ color: 'var(--text-primary)' }}>Assigned Doctor</h3>
                                <div className="flex items-center gap-4 mb-4">
                                    <div className="w-12 h-12 rounded-full gradient-primary flex items-center justify-center text-white font-bold text-xl">
                                        {patient.doctor?.fullName?.charAt(0) || 'D'}
                                    </div>
                                    <div>
                                        <p className="font-semibold" style={{ color: 'var(--text-primary)' }}>{patient.doctor?.fullName || 'N/A'}</p>
                                        <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{patient.doctor?.specialization || 'Doctor'}</p>
                                    </div>
                                </div>
                                <div className="space-y-3">
                                    <div className="flex items-center gap-3 text-sm">
                                        <HiOutlinePhone className="text-primary" />
                                        <span style={{ color: 'var(--text-secondary)' }}>{patient.doctor?.phone || 'N/A'}</span>
                                    </div>
                                    {patient.doctor?.hospital && (
                                        <div className="flex items-center gap-3 text-sm">
                                            <HiOutlineOfficeBuilding className="text-secondary" size={16} />
                                            <span style={{ color: 'var(--text-secondary)' }}>{patient.doctor.hospital}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="space-y-4">
                            {!patient.nutritionSchedule ? (
                                <div className="glass-card p-12 text-center">
                                    <p style={{ color: 'var(--text-muted)' }}>No nutrition schedule assigned yet.</p>
                                </div>
                            ) : (
                                <div className="glass-card p-6 space-y-6">
                                    {(patient.nutritionSchedule.restrictions?.length > 0 || patient.nutritionSchedule.specialInstructions) && (
                                        <div className="space-y-2">
                                            {patient.nutritionSchedule.restrictions?.length > 0 && (
                                                <div>
                                                    <h4 className="text-sm font-semibold mb-2" style={{ color: 'var(--text-muted)' }}>Restrictions</h4>
                                            ...
                                            {patient.nutritionSchedule.specialInstructions && (
                                                <div>
                                                    <h4 className="text-sm font-semibold mb-1" style={{ color: 'var(--text-muted)' }}>Special Instructions</h4>
                                                    <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{patient.nutritionSchedule.specialInstructions}</p>
                                                </div>
                                            )}
                                        </div>
                                    )}
                                    {['breakfast', 'morningSnack', 'lunch', 'eveningSnack', 'dinner'].map((meal) => {
                                        ...
                                        return (
                                            <div key={meal} className="border rounded-xl p-4" style={{ borderColor: 'var(--border)' }}>
                                                <h4 className="font-semibold capitalize mb-3" style={{ color: 'var(--text-primary)' }}>{meal.replace(/([A-Z])/g, ' $1').trim()}</h4>
                                                <div className="space-y-2">
                                                    {items.map((item, i) => (
                                                        <div key={i} className="flex justify-between text-sm">
                                                            <span style={{ color: 'var(--text-secondary)' }}>{item.name} {item.quantity && `(${item.quantity})`}</span>
                                                            <span style={{ color: 'var(--text-muted)' }}>{item.calories || 0} cal</span>
                                                        </div>
                                                    ))}
                                                    <div className="flex justify-between text-sm font-medium pt-2 mt-2 border-t" style={{ borderColor: 'var(--border)', color: 'var(--primary)' }}>
                            ...
                            <div className="space-y-3">
                                {(patient.recoveryLogs || []).slice(0, 10).map(log => (
                                    <div key={log._id} className="glass-card p-4">
                                        ...
                                        {log.symptoms?.length > 0 && <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>Symptoms: {log.symptoms.join(', ')}</p>}
