                        ...
                        <div className="glass-card p-6 space-y-5 border border-[var(--glass-border)] rounded-3xl">
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={l} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1">
                                        <span className="text-[var(--text-muted)] font-medium text-sm">{l}</span>
                                        <span className="font-bold text-[var(--text-secondary)] text-sm sm:text-right break-words max-w-[70%]">{v || 'Not Available'}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            {(patient.recoveryLogs || []).slice(0, 10).map((log, i) => (
                                <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} key={log._id} className="glass-card p-5 border border-[var(--border)] rounded-2xl bg-[var(--bg-secondary)] hover:shadow-md transition-shadow">
                                    ...
                                    {log.symptoms?.length > 0 && <p className="text-xs text-[var(--text-secondary)] mb-1">Symptoms: {log.symptoms.join(', ')}</p>}
                                    ...
                                    {log.message && <div className="mt-2 bg-[var(--bg-tertiary)]/50 p-3 rounded-xl border border-[var(--border)]/50"><p className="text-sm font-medium text-[var(--text-secondary)] italic">"{log.message}"</p></div>}
                                    {log.notes && <p className="text-xs mt-1 italic text-[var(--text-muted)]">{log.notes}</p>}
                                </motion.div>
                            ))}
